...
> \[!CAUTION\] This library is in early development and the API is
> subject to change. The core functionality of downloading and
> visualizing tidal hindcast data at specific points is stable, but
> additional features and datasets are still being added. Please reach
> out if you have questions or would like to contribute!

> \[!IMPORTANT\] At this time this libary does not support for the U.S.
> DOE H20 wave energy hindcast dataset. The marine and hydrokinetic
> toolkit (MHKiT) has a [wave hindcast
...
from the sea surface to the seafloor. It is the primary dataset for IEC
62600-201 Stage 1 reconnaissance-level tidal energy site assessment in
the US.
...
spread and whiskers; a depth vs. speed scatter coloured by direction
with quadratic mean and maximum fit curves; and per-layer histograms of
...
visualisations — time series, velocity exceedance, and joint probability
...
Each panel covers the full hindcast year at one site. The colourbar
range is shared across all six: `vmax` is the maximum speed in the
dataset, rounded up to the nearest 0.5 m/s.
...
2 × 3 grid of JPD polar histograms with a shared colour scale.
...
Installing via pip includes

``` bash
# Point query — returns nearest grid point metadata
us-tidal-query --lat 60.73 --lon -151.43

# Area query — returns all grid points within a bounding box
us-tidal-query --mode area --lat-min 60.7 --lat-max 60.8 \
               --lon-min -151.5 --lon-max -151.4

# Line query — returns grid points along a transect
...
`tidal_hindcast` is a facade over lower-level classes that remain
accessible for finer spatial control or custom processing pipelines:

``` python
from us_marine_energy_resource import TidalManifestQuery
from us_marine_energy_resource import tidal_hindcast as tidal
...
cache = tidal._state.cache   # initialised on first get_data_at_point() call
query = tidal._state.manifest

# Spatial queries against the manifest
point = query.query_nearest_point(lat=60.73, lon=-151.43)
area  = query.query_all_within_rectangular_area(60.7, 60.8, -151.5, -151.4)
line  = query.query_all_on_line(60.7, -151.4, 60.8, -151.5)

# Load a specific grid point's time-series parquet
